...
adminMissionsRouter.get('/:id/loadClassifiedQuestArtists', async (req, res) => {
    ...
    if (mission.isArtistShowcase && mission.showcaseMissionArtists?.length) {
        const seen = new Set();
        for (const entry of mission.showcaseMissionArtists) {
...
adminMissionsRouter.post('/:id/toggleIsGenreShowcase', async (req, res) => {
    await mission_1.MissionModel.findByIdAndUpdate(req.params.id, { isGenreShowcase: req.body.isGenreShowcase }).orFail();
    res.json(req.body.isGenreShowcase);
});
/* POST add genre option */
adminMissionsRouter.post('/:id/addGenreOption', async (req, res) => {
    await mission_1.MissionModel.findByIdAndUpdate(req.params.id, { $addToSet: { genreOptions: req.body.genreOption } }).orFail();
    const mission = await mission_1.MissionModel.findById(req.params.id).orFail();
    res.json(mission.genreOptions);
});
/* POST remove genre option */
adminMissionsRouter.post('/:id/removeGenreOption', async (req, res) => {
    await mission_1.MissionModel.findByIdAndUpdate(req.params.id, { $pull: { genreOptions: req.body.genreOption } }).orFail();
    const mission = await mission_1.MissionModel.findById(req.params.id).orFail();
    res.json(mission.genreOptions);
});
...
adminMissionsRouter.post('/:id/toggleIsUniqueArtistToRanked', async (req, res) => {
    await mission_1.MissionModel.findByIdAndUpdate(req.params.id, { isUniqueArtistToRanked: req.body.isUniqueArtistToRanked }).orFail();
...
adminMissionsRouter.post('/:id/calculateUserPoints', async (req, res) => {
    ...
    for (const id of [...userIds]) {
        const user = await user_1.UserModel.findById(id).orFail();
        const pointsBefore = user.totalPoints;
        const pointsAfter = await (0, points_1.updateUserPoints)(id);
