...
const adminBackgroundsRouter = express.Router();

adminBackgroundsRouter.use(isLoggedIn);
adminBackgroundsRouter.use(isAdmin);
...
adminBackgroundsRouter.post('/:id/updateUser', async (req, res) => {
    const user = await UserModel.findOne().byUsernameOrOsuId(req.body.user);
...
adminBackgroundsRouter.post('/:id/delete', async (req, res) => {
    await BackgroundModel.findByIdAndDelete(req.params.id).orFail();

    res.json({ success: 'ok' });
});

export default adminBackgroundsRouter;
